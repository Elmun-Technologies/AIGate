...
class ProviderUsageEvent(Base):
    ...
    tool_call_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tool_calls.id"), nullable=True, unique=True)
    ...
    cost_usd: Mapped[decimal.Decimal] = mapped_column(Numeric(12, 6), nullable=False, default=decimal.Decimal("0"))
    tokens_in: Mapped[int | None] = mapped_column(Integer, nullable=True)
    tokens_out: Mapped[int | None] = mapped_column(Integer, nullable=True)
    shadow_ai_usage: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
